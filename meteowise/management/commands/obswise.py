...
class Command(BaseCommand):
	...
	def handle(self, *args, **options):
		...
		now = datetime.now()
		print(now.strftime("[%Y-%m-%d %H:%M:%S]")+" INFO Calcul de l observation des 24 heures passées depuis "+date_ref.strftime('%Y-%m-%d 09h00'))
		...
		req = f"""WITH parsed_data AS (
			...
			FROM parsed_data
			WHERE phenomenon_timestamp >= date_trunc('day', TIMESTAMP '{date_sql}') + interval '9 hour' - interval '1 day' 
			  AND phenomenon_timestamp <  date_trunc('day', TIMESTAMP '{date_sql}') + interval '9 hour'
