...
class Command(BaseCommand):
	...
	def add_arguments(self, parser):
		...
		parser.add_argument('--start', type=str, help="Echéance de début de calcul à partir de la date selectionnée")
		parser.add_argument('--end', type=str, help="Echéance de fin de calcul à partir de la date et echéance selectionnées (négative pour revenir en arrière)")
	...
	def handle(self, *args, **options):
		...
		if start_str:
			try:
				...
				self.stderr.write(self.style.ERROR("Echeance début invalide. un entier est attendu"))
				return
			if start<-240 or start >240 :
				self.stderr.write(self.style.ERROR("Seul 10 jours d'échance sont permises"))
				return
		else:
			self.stderr.write(self.style.ERROR("Echéance invalide. un entier est attendu"))
			return
		...
		req = f"""WITH parsed_data AS (
			...
			FROM parsed_data
			WHERE phenomenon_timestamp >= date_trunc('day', TIMESTAMP '{date_sql}') + interval '{start} hours' 
			  AND phenomenon_timestamp <  date_trunc('day', TIMESTAMP '{date_sql}') + interval '{end} hour'
		...
		try :
			ech = Echeance.objects.get(start=start,end=end)
		except Exception as e :
			ech = Echeance.objects.get(start=end,end=start)
