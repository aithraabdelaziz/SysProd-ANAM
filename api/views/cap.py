...
class CAPViewSet(viewsets.ViewSet):
    """
    ViewSet pour lister ou télécharger les fichiers CAP (vc ou vf).
    """
    ...
    def list_files(self, request):
        ...
        folder_path = os.path.join(settings.MEDIA_ROOT, 'vigilance','cap_xmls', date)
        ...
        for f in os.listdir(folder_path):
            if f.endswith(".xml"):
                if param == "*":
                    if "-" in f:
                        file_url = f"{base_url}vigilance/cap_xmls/{date}/{quote(f)}"
    ...
    @extend_schema(
        parameters=[
            OpenApiParameter(name='date', location=OpenApiParameter.QUERY, description='Date format YYYYmmdd', required=True, type=str),
            OpenApiParameter(name='filename', location=OpenApiParameter.QUERY, description='Nom du fichier XML', required=True, type=str),
        ],
        responses={200: None}
        # responses={200: 'application/xml', 400: dict, 404: dict}
    )
    ...
    def download_file(self, request):
        ...
        file_path = os.path.join(settings.MEDIA_ROOT, 'vigilance','cap_xmls', date, filename)
